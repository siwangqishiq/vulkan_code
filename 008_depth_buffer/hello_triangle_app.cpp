...
void HelloTriangleApp::createTextureSampler(){
    ...
    std::cout << "physical deivice prop : " << props.limits.maxSamplerAnisotropy << std::endl;
...
void HelloTriangleApp::createDepthResource(){
    std::cout << "create depth resource" << std::endl;
...
VkFormat HelloTriangleApp::findSupportedFormat(
    const std::vector<VkFormat> &candicates,
...
void HelloTriangleApp::createTextureImage(){
    ...
    std::cout << "read texture file w = " << texWidth 
            << " h = " << texHeight << " channels : " << texChannels << std::endl;
...
void HelloTriangleApp::copyBufferToImage(VkBuffer buffer, VkImage image, 
...
bool HelloTriangleApp::isDeviceSuitable(VkPhysicalDevice phyDevice){
    ...
    vkGetPhysicalDeviceFeatures(phyDevice, &supportFeatures);

    std::cout << "Support Feature samplerAnisotropy : " << supportFeatures.samplerAnisotropy << std::endl;
    std::cout << "Support Feature Geometry shader : " << supportFeatures.geometryShader << std::endl;
...
void HelloTriangleApp::createLogicDevice(){
    ...
    phyFeatures.samplerAnisotropy = VK_TRUE;//各向异性过滤设置打开
...
void HelloTriangleApp::createRenderPass(){
    ...
    std::array<VkAttachmentDescription , 2> attachs = {colorAttach, depthAttachment};
    createInfo.attachmentCount = static_cast<uint32_t>(attachs.size());
...
void HelloTriangleApp::createGraphPipline(){
    ...
    // inputAssembleCreateInfo.topology = VK_PRIMITIVE_TOPOLOGY_LINE_STRIP;
...
void HelloTriangleApp::copyBuffer(VkBuffer srcBuf, VkBuffer dstBuf, VkDeviceSize size){
    VkCommandBuffer transferCmdBuffer = beginSingleTimeCommands();
    VkBufferCopy regions{};
    regions.size = size;
    regions.srcOffset = 0;
    regions.dstOffset = 0;
    vkCmdCopyBuffer(transferCmdBuffer, srcBuf, dstBuf, 1, &regions);
    endSingleTimeCommands(transferCmdBuffer);
}
...
std::array<VkVertexInputAttributeDescription , 3> Vertex::attributeDesc(){
